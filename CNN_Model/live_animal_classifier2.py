#!/usr/bin/env python3
import cv2, os, time, sys, argparse, torch
...
def classify_frame(model, frame, transform, threshold):
    ...
    return ("ANIMAL" if confidence >= threshold else "NOT ANIMAL"), confidence

def run_single_inference(model, transform, threshold):
    ...
    if not ret:
        ...
        sys.exit(1)
    result, confidence = classify_frame(model, frame, transform, threshold)
    if result == "ANIMAL":
...
def main():
    ...
    if args.mode == 'single_inference':
        run_single_inference(model, transform, args.confidence_threshold)
        return
    ...
    last_drive_time = 0
    COOLDOWN = 5.0
    car_path = os.path.join(script_dir, 'Car_Controller.py')
    ...
    try:
        while True:
            ...
            if result == "ANIMAL" and (time.time() - last_drive_time) > COOLDOWN:
                print(f"[ALERT] ANIMAL ({confidence:.2f})", file=sys.stderr)
                import subprocess
                subprocess.Popen(['python3', car_path])
